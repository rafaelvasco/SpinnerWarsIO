...
func join_game(url : String, game : String) -> int:
	stop()
	hosting = false
	self.game = game
	return ws_client.connect_to_url(url)
...
func stop():
	if rtc_mp.get_connection_status() != WebRTCMultiplayer.CONNECTION_DISCONNECTED:
		rtc_mp.close()
